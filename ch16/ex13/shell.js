...
async function runcmd(cmd, stdin = null, stdout = null) {
  switch (cmd.type) {
    ...
    case ">": // RedirCmd 出力リダイレクト
      {
        // FIXME: ここを実装してね (2行程度)
        // HINT: cmd.file のストリームを createWriteStream で作成し runcmd を再帰的に呼び出す
        const writeStream = fs.createWriteStream(cmd.file);
        await runcmd(cmd.cmd, stdin, writeStream); // cmd.cmdが標準出力に書き出したデータが、コンソールではなく、writeStreamに流れるようにする
      }
      break;

    case "<": // RedirCmd 入力リダイレクト
      {
        // FIXME: ここを実装してね (2行程度)
        // HINT: cmd.file のストリームを createReadStream で作成し runcmd を再帰的に呼び出す
        const readStream = fs.createReadStream(cmd.file);
        await runcmd(cmd.cmd, readStream, stdout); // readStreamから読み込んだデータがcmd.cmdの標準入力に流れるようにする
      }
      break;
    ...
    case "|": // PipeCmd
      {
        ...
        const passThrough = new PassThrough(); // 左のstdoutと右のstdinを繋ぐストリームを作成
        await Promise.all([
